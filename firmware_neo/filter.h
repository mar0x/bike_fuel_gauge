...
struct filter_t {
    ...
    filter_t(double max_cov = 0.05) : max_cov_(max_cov) { }
...
inline
unsigned long filter_t::update(unsigned long v, unsigned long t) {
    last_update = t;
...
inline
void filter_t::hist_push(unsigned long v) {
    hist_ring[hist_id] = v;
    hist_id = (hist_id + 1) % HIST_SIZE;
}

inline
double filter_t::mean() const {
    double s = 0;
...
inline
double filter_t::cov(double m) const {
    double variance = 0;
